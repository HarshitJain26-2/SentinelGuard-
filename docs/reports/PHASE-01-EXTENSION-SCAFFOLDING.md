...
### 16.8. Actual Manual Test Execution & Observed Results

Manual testing was conducted in Google Chrome on the unpacked SentinelGuard extension with the static local test server (`python -m http.server 3000 --directory test-page`). All actual results are recorded below:

1. **Extension Loading: PASS**
   - SentinelGuard loaded cleanly into Google Chrome via `chrome://extensions/` in Developer Mode from unpacked directory `c:\Users\Harshit\Desktop\Projects\ERI\SentinelGuard-\extension`.
   - The extension card populated without warnings, manifest errors, or icon rendering failures.

2. **Popup UI: PASS**
   - Clicking the toolbar extension icon opens the popup immediately.
   - Popup rendered at fixed 300px width with dark cyber-defense theme.
   - Verified UI components:
     - Header brand: **SentinelGuard** / *Bot & Credential Defense*
     - Status badge: **ACTIVE**
     - Protection status text: **ON**
     - Interactive toggle switch: Clicking switch smoothly transitions text to **OFF**, badge to **PAUSED**, and updates DevTools console without page reload.

3. **Background Service Worker: PASS**
   - Opened service worker DevTools via `chrome://extensions/` > *Inspect views: service worker*.
   - Console log confirmed:
     ```
     [SentinelGuard] Service worker initialized.
     [SentinelGuard] Extension installed successfully.
     ```
   - Zero uncaught exceptions, runtime errors, or unhandled promise rejections observed.

4. **Local Test Environment: PASS**
   - Local test harness started via:
     ```powershell
     python -m http.server 3000 --directory test-page
     ```
   - Page was successfully accessible at `http://localhost:3000/`.

5. **Content Script Injection: PASS — MANUALLY VERIFIED**
   - Webpage DevTools Console (`F12` on `http://localhost:3000/`) showed:
     ```
     [SentinelGuard] Content script loaded.
     ```
   - *Inspection observation:* DevTools *Sources > Content scripts* initially showed no scripts when `localhost:3000` was an unhandled Chrome error page (`chrome-error://chromewebdata/`). After starting the local test server and reloading the page, the content script was injected and verified successfully under Sources and Console.

6. **URL Verification: PASS**
   - Verified active document URL was:
     `http://localhost:3000/`
   - Confirmed the page was NOT redirected to browser-internal error page:
     `chrome-error://chromewebdata/`

7. **Network Isolation: PASS**
   - Chrome DevTools *Network* tab showed the local document request for `/index.html`.
   - Exactly zero backend, telemetry, or API requests were made by SentinelGuard.
   - The only observed 404 was `favicon.ico`, which is unrelated to SentinelGuard and expected because the minimal static test page does not declare a favicon.

8. **Behavioral Signal Capture: PASS / EXPECTED ABSENCE**
   - Verified the intentional absence of:
     - Mouse movement listeners (`mousemove`, trajectory sampling)
     - Keyboard / keydown capture (dwell time, flight time)
     - Password or credential field interception
     - Form input value capture
     - Login submission hooks
     - Backend event transmission (`POST /api/v1/events/`)
     - Machine learning risk evaluation
   - This absence is intentional and aligns strictly with the Phase 1 scaffold boundary; behavioral signal capture belongs to later phases.
...
| Local test page accessibility | Built-in Python HTTP server | `http://localhost:3000/` serves 200 OK | **PASS** |
| Content script injection on `http://localhost:3000/` | Manual browser inspection on `test-page` | `[SentinelGuard] Content script loaded.` in Console | **PASS — MANUALLY VERIFIED** |
| Zero telemetry / network calls | Chrome DevTools Network inspection | 0 extension requests to backend | **PASS** |
...
*Phase 1 Status: ✅ COMPLETE (Manually Verified in Chrome)*  
